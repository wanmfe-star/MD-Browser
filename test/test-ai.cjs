...
(async()=>{
 ...
 try{
  ...
  let c=await call('create');await call('send',{id:c.id,text:'解释这段话',sources:[{id:'doc',name:'文档.md',path:'/私人目录/文档.md',text:'这是原文',mode:'selection'}]});c=await done(c.id);assert.equal(c.messages.at(-1).status,'complete');assert.ok(c.messages.at(-1).content.includes('中文回答'));assert.ok(requests.at(-1).body.messages.at(-1).content.includes('这是原文'));
  assert.ok(!JSON.stringify(requests.at(-1).body).includes('文档.md'));assert.ok(!JSON.stringify(requests.at(-1).body).includes('私人目录'));assert.ok(requests.at(-1).body.messages.at(-1).content.includes('引用 1 · 选文'));
  ...
  assert.ok(requests.every(r=>!JSON.stringify(r.body).includes('长文.pdf')),'long-text analysis omits filename in every request');
